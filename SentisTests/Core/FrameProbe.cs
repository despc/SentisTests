...
namespace SentisTests.Core
{
    ...
    public static class FrameProbe
    {
        ...
        public static void Install(PatchManager patchManager)
        {
            ...
            // What a character costs the server: its own updates and its jetpack.
            var character = typeof(Sandbox.Game.Entities.Character.MyCharacter);
            Hook(ctx, First(character, "UpdateBeforeSimulation"), nameof(CharUpdatePrefix), nameof(CharUpdateSuffix));
            Hook(ctx, First(character, "UpdateAfterSimulation"), nameof(CharUpdateAfterPrefix), nameof(CharUpdateAfterSuffix));
            Hook(ctx, First(character, "Simulate"), nameof(CharSimulatePrefix), nameof(CharSimulateSuffix));
            Hook(ctx, First(character, "UpdateBeforeSimulation10"), nameof(CharUpdate10Prefix), nameof(CharUpdate10Suffix));
            Hook(ctx, First(character, "UpdateBeforeSimulation100"), nameof(CharUpdate100Prefix), nameof(CharUpdate100Suffix));
            Hook(ctx, First(character, "UpdateBeforeSimulationParallel"), nameof(CharParallelPrefix), nameof(CharParallelSuffix));
            Hook(ctx, typeof(MyShipDrill).Assembly.GetType("Sandbox.Game.Entities.Character.Components.MyCharacterJetpackComponent")
                    ?.GetMethod("UpdateSimulate", any),
                nameof(CharJetpackPrefix), nameof(CharJetpackSuffix));
            ...
            Hook(ctx, sandbox.GetType("Sandbox.Game.Entities.Blocks.MyPistonBase")?.GetMethod("UpdateBeforeSimulation", any, null, Type.EmptyTypes, null),
                nameof(PistonUpdatePrefix), nameof(PistonUpdateSuffix));
        ...
        private static void FrameSuffix()
        {
            ...
            for (var i = 0; i < (int)Section.Count; i++)
            {
                ...
                if (!IsNested(i)) attributed += sectionMs;
        ...
        private static void CharUpdatePrefix() => Begin(Section.CharUpdate);
        private static void CharUpdateSuffix() => End(Section.CharUpdate);
        private static void CharUpdateAfterPrefix() => Begin(Section.CharUpdateAfter);
        private static void CharUpdateAfterSuffix() => End(Section.CharUpdateAfter);
        private static void CharSimulatePrefix() => Begin(Section.CharSimulate);
        private static void CharSimulateSuffix() => End(Section.CharSimulate);
        private static void CharUpdate10Prefix() => Begin(Section.CharUpdate10);
        private static void CharUpdate10Suffix() => End(Section.CharUpdate10);
        private static void CharUpdate100Prefix() => Begin(Section.CharUpdate100);
        private static void CharUpdate100Suffix() => End(Section.CharUpdate100);
        private static void CharJetpackPrefix() => Begin(Section.CharJetpack);
        private static void CharJetpackSuffix() => End(Section.CharJetpack);
        private static void CharParallelPrefix() => Begin(Section.CharParallel);
        private static void CharParallelSuffix() => End(Section.CharParallel);
        ...
        private static void PistonUpdatePrefix() => Begin(Section.PistonUpdate);
        private static void PistonUpdateSuffix() => End(Section.PistonUpdate);
            ...
            long[][] frameSections, long[] frameAlloc)
        {
            ...
            void Bucket(string label, List<int> idx)
            {
                ...
                {
                    var attributed = 0.0;
                    for (var i = 0; i < (int)Section.Count; i++) if (!IsNested(i)) attributed += a[i];
                    return attributed;
                }
